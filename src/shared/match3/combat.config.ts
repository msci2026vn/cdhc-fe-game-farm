...
// Archetype-based skill names count
export const ARCHETYPE_SKILL_COUNTS: Record<string, number> = {
  glass_cannon: 3,
  tank: 3,
  healer: 3,
  assassin: 3,
...
export function getBossSkillName(archetype: string): string {
  const arcKey = ARCHETYPE_SKILL_COUNTS[archetype] ? archetype : 'none';
  const count = ARCHETYPE_SKILL_COUNTS[arcKey];
  const index = Math.floor(Math.random() * count);
  return `campaign.boss_random_skills.${arcKey}.${index}`;
}
