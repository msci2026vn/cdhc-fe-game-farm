...
export function StoryDrawer({ isOpen, onClose, storyFull, bossName }: StoryDrawerProps) {
  ...
  return (
    <BottomDrawer isOpen={isOpen} onClose={onClose} title={`📖 Truyền thuyết`}>
      <p className="text-sm text-yellow-400 italic mb-4">{bossName}</p>

      {parts.length === 0 ? (
        <p className="text-sm text-gray-500">Chưa có truyền thuyết.</p>
      ) : (
        <div className="flex flex-col gap-4">
          {parts.map((part, i) => {
            ...
            return organic ? (
              ...
              >
                <p className="text-xs text-green-400 font-bold mb-1">🌿 Phòng trừ sinh học</p>
