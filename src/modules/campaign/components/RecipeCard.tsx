  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-2.5 text-[11px]">
        {recipe.yieldBonus > 0 && (
          <span className="text-green-400">
            +{Math.round(recipe.yieldBonus * 100)}% yield
          </span>
        )}
        {recipe.timeReduction > 0 && (
          <span className="text-blue-400">
            -{Math.round(recipe.timeReduction * 100)}% time
          </span>
        )}
      ...
      <div className="text-[10px] text-white/40 mb-2">
        Zone {recipe.zoneNumber}: {ZONE_NAMES[recipe.zoneNumber] ?? `Zone ${recipe.zoneNumber}`}
      </div>
        ...
        {ingredientChecks.map((ing, i) => (
          <div key={i} className="flex items-center gap-2 text-[11px]">
            ...
            <span className="text-white/70 truncate flex-1">
              {ing.name} {ing.isSameZone ? '' : '(khác zone)'}
            </span>
